...
if not os.path.isdir('Rec_%s' % TARGET):
    os.mkdir('Rec_%s' % TARGET)
...
state_dict = torch.load('ckpt/34_087.pth', map_location=device)
...
with torch.no_grad():
    ...
    for ind2, (vx, vfn) in enumerate(val_loader):
        ...
        for bt in range(val_batch_size):
            ...
            np.save('Rec_%s/%s.npy' % (TARGET, save_path), img[bt])
